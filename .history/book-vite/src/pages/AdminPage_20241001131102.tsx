...
import { addBook, removeBook, updateBook, removeFeaturedBook, removeBestSellerBook } from '../redux/bookSlice'; // Thêm các action vào đây
...
const AdminPage: React.FC = () => {
  ...
  // Hàm thêm sách vào danh sách nổi bật
  const handleAddFeatured = (book: Book) => {
    dispatch(removeFeaturedBook(book.id)); // Cập nhật trạng thái trong Redux
    message.success(`Sách "${book.title}" đã được thêm vào danh sách nổi bật!`);
  };

  // Hàm xóa sách khỏi danh sách nổi bật
  const handleRemoveFeatured = (bookId: number) => {
    dispatch(removeFeaturedBook(bookId)); // Cập nhật trạng thái trong Redux
    message.success('Sách đã được xóa khỏi danh sách nổi bật!');
  };

  // Hàm thêm sách vào danh sách bán chạy
  const handleAddBestSeller = (book: Book) => {
    dispatch(removeBestSellerBook(book.id)); // Cập nhật trạng thái trong Redux
    message.success(`Sách "${book.title}" đã được thêm vào danh sách bán chạy!`);
  };

  // Hàm xóa sách khỏi danh sách bán chạy
  const handleRemoveBestSeller = (bookId: number) => {
    dispatch(removeBestSellerBook(bookId)); // Cập nhật trạng thái trong Redux
    message.success('Sách đã được xóa khỏi danh sách bán chạy!');
  };
      ...
      <Tabs defaultActiveKey="1">
        ...
        <TabPane tab="Danh Sách Sách" key="2">
          ...
          <BookListAdmin
            ...
            onRemoveBestSeller={handleRemoveBestSeller}
          />
        </TabPane>
