...
const EditBookModal: React.FC<EditBookModalProps> = ({ visible, book, onSave, onCancel }) => {
  ...
  const [isOnPromotion, setIsOnPromotion] = React.useState<boolean>(false);
  ...
  React.useEffect(() => {
    if (book) {
      ...
      setIsFeatured(!!book.isFeatured); // Chuyển đổi thành boolean
      setIsOnPromotion(!!book.isBestSeller); // Chuyển đổi thành boolean
    }
  }, [book, form]);
