...
const BookList: React.FC = () => {
  ...
  useEffect(() => {
    const getBooks = () => {
      if (grade) {
        return booksByGrade[grade] || [];
      } else {
        return Object.values(booksByGrade).flat();
      }
    };
    
    const books = getBooks();
    setFilteredBooks(shuffleArray(books)); // Sắp xếp ngẫu nhiên sách
  }, [booksByGrade, grade]);
  ...
  const filterBooks = (grade?: string, term: string = '') => {
    ...
    setFilteredBooks(shuffleArray(updatedBooks)); // Sắp xếp ngẫu nhiên sau khi lọc
  };
              ...
              <Card
                ...
                cover={
                  <img
                    alt={book.title}
                    src={book.image}
                    style={{ height: '250px', objectFit: 'cover', paddingTop: 12 }}
                  />
                }
