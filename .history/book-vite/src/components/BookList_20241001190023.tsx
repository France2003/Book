...
const BookList: React.FC = () => {
  ...
  const promotionalBooks = useSelector((state: RootState) => state.books.bestSellers); // Lấy sản phẩm giảm giá từ store
  ...
  const filterBooks = (grade?: string, term: string = '') => {
    ...
    // Kết hợp với các sản phẩm giảm giá
    const allBooks = updatedBooks.concat(promotionalBooks);

    // Xóa trùng lặp theo id sản phẩm
    const uniqueBooks = Array.from(new Set(allBooks.map(book => book.id)))
      .map(id => allBooks.find(book => book.id === id))
      .filter((book): book is Book => !!book); // Giữ lại chỉ những sách không undefined
              ...
              >
                <div className="flex flex-col justify-between h-full">
                  <div>
                    ...
                    {/* Hiển thị chữ "Đã giảm giá: 0.2%" nếu sản phẩm giảm giá */}
                    {book.isPromotional && (
