            ...
            <div className="mt-8">
                {reviews.map((review) => (
                    <div key={review.id} className="mb-6 leading-9">
                        <p className=" text-gray-600"> Tên người dùng: <b className='pl-3'>{user.name}</b></p>
                        <div className="flex items-center">
                            <Rate disabled defaultValue={review.rating} />
                        </div>
                        <p className="text-gray-800">Chất lượng sản phẩm: <b>{review.content}</b></p>
                        <p className="text-sm text-gray-500"> Thời điểm đánh giá: <b>{new Date(review.date).toLocaleString()}</b></p>
                    </div>
                ))}
            </div>
        </div>
